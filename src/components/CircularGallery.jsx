...
function createTextTexture(gl, text, font = "600 24px Montserrat, sans-serif", color = "white") {
...
`,
    ...
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.6 - 0.06;
...
void main() {
  ...
  // Vignette e iluminación sutil para mayor profundidad
  vec2 centered = vUv - 0.5;
  float r = length(centered) * 1.2;
  float vignette = smoothstep(0.85, 0.35, r);
  color.rgb *= mix(0.88, 1.0, vignette);

  // Bordes redondeados con antialias
  float d = roundedBoxSDF(vUv - 0.5, vec2(0.5 - uBorderRadius), uBorderRadius);
  float edgeSmooth = 0.002;
  float alpha = 1.0 - smoothstep(-edgeSmooth, edgeSmooth, d);

  // Sombra interna sutil cerca del borde
  float inner = smoothstep(0.0, 0.02, abs(d));
  color.rgb *= mix(0.96, 1.0, inner);

  // Gamma leve para un look más suave
...
`,
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Escala profesional con aspecto 4:3 y altura responsiva
    const isSmall = this.screen.width < 640;
    const isMedium = this.screen.width < 1024;
    const heightFactor = isSmall ? 0.44 : (isMedium ? 0.5 : 0.56);
    const planeH = this.viewport.height * heightFactor;
    const aspect = 4.0 / 3.0;
    this.plane.scale.y = planeH;
    this.plane.scale.x = planeH * aspect;

    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];

    this.padding = this.viewport.width * 0.04; // 4% del ancho del viewport
